def flood_fill(input, x, y, fill_value):
    """
    Recursively replaces all adjacent cells with the value 5 starting from (x, y) with fill_value.
    """
    # Base case: check for out-of-bounds or if the cell is not 5
    if x < 0 or x >= len(input) or y < 0 or y >= len(input[0]) or input[x][y] != 5:
        return
    
    # Replace the current cell with fill_value
...
def solve(input):
    """
    Transforms the input grid by replacing clusters of 5s with incrementing numbers starting from 1.
    """
